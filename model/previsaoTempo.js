...
class PrevisaoTempo {
    ...
    insertPrevisaoTempo(tempo) {
        const startDate = tempo.data.timelines[0].startTime.substring(0, 19).replace('T', ' ');
        const endDate = tempo.data.timelines[0].endTime.substring(0, 19).replace('T', ' ');

        const lista = tempo.data.timelines[0].intervals;
        for (var i = 0; i < lista.length; i++) {
            var temperatureDate = lista[i].startTime.substring(0, 19).replace('T', ' ');
